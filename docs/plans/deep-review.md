...
- **G1 — The degrade loop overwrites sections that were successfully written.**
...
- **G2 — Degraded placeholders fabricate readable data.** `research-engine.ts:851-896`
...
- **H1 — `markHeld` is a blind write.** `jobs/firestore.ts:197`. **Reproduced.**
...
- **H2 — `setJobSlotHeld` is a blind write.** `slots.ts:115-117`. **Reproduced.** A
...
- **I1 — `refundForJob` does not read the job's status.** `credits/store.ts:157-176`.
...
- **I2 — `refundForJob` credits the caller's `(appId, userId)`,** while the amount
