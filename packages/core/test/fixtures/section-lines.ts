...
export type SectionLineKey = 'degradedSection' | 'unenrichedSection' | 'reconstructedSection';

/**
 * Canonical per-section lines.
 *
 *   - `degradedSection` — the body was suppressed; say so and say the rest is
 *     intact. (English carried an extra "below" in the viewer only; no other
 *     language had a word for it, so it is gone.)
...
export const SECTION_LINES: Record<'en' | 'es' | 'fr' | 'pt', Record<SectionLineKey, string>> = {
  en: {
    degradedSection: 'We could not complete this section for this report. Everything else was researched and written as usual.',
  ...
  es: {
    degradedSection: 'No pudimos completar esta sección para este informe. Todo lo demás se investigó y redactó con normalidad.',
  ...
  fr: {
    degradedSection: 'Nous n’avons pas pu terminer cette section pour ce rapport. Tout le reste a été recherché et rédigé normalement.',
  ...
  pt: {
    degradedSection: 'Não conseguimos concluir esta seção deste relatório. Todo o restante foi pesquisado e redigido normalmente.',
