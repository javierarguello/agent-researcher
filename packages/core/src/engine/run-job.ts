...
import { markCompleted, markFailed, markRunning, setProgress } from '../jobs/firestore.js';
      ...
      onTrace: async (trace) => {
        ...
        for (const a of trace.agents) {
          ...
          if (a.status === 'failed') {
            ...
            log.info('agent.ok', { agentId: a.id, wave: a.wave, model: a.model, turnsUsed: a.turnsUsed, produced: a.produces });
          }
        }
      },
    });
