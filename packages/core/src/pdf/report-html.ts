...
function mdInline(s: string): string {
  ...
  out = out.replace(
    ...
    // The label goes through `linkLabel`: a citation whose text is its own url is
    // shown as the host. `t` is already escaped (`esc(s)` ran over the whole string
    // first) and the host class stops at the first `/`, so what comes back is still
    // escaped.
    (_m, t, u) => `<a href="${u}">${linkLabel(t, u)}</a>`,
  );
  // AFTER the link rule, so what is left is a tag with nothing behind it: the ones
  // that labelled a link were consumed above and resolved to their host.
  out = stripEvidenceTags(out);
 ...
 * (round 10, R10-8).
 *
 * Keep identical to its twin in `apps/fbizlab/src/lib/safe-href.ts`; the two
  ...
  <section class="pg" id="sec-${esc(s.key)}">
    <div class="body">
      ...
      <div class="seccontent">${
        ...
        degraded.has(s.key)
          ? `<p class="soft">${esc(l.degradedSection ?? '')}${statuses.length === 1 ? ` ${esc(l.allElseOk ?? '')}` : ''}</p>`
